...
import {
  ...
  useFieldError,
  useFormState,
} from "@/components/ui/action-form";
import { Button, ButtonLink } from "@/components/ui/button";
import { Checkbox, Input } from "@/components/ui/form";
import { Alert, Badge } from "@/components/ui/primitives";
import { ShareMenu } from "@/components/campaign/share-menu";
...
export function GetLinkPanel({
  ...
  /** Brand-approved copy, prefilled into the share targets that accept text. */
  suggestedText: string;
  /** Hides the advanced tracking fields during a walkthrough. */
  presenting?: boolean;
}) {
    ...
    return (
      <div className="card p-5">
        ...
        <p className="mt-1.5 text-sm text-fg-muted text-pretty">
          Sign in as a creator or publisher to generate a link for this
          campaign.
        </p>
  ...
  if (!isCreator) {
    return (
      <div className="card p-5">
        ...
        <p className="mt-1.5 text-sm text-fg-muted text-pretty">
          You are signed in with a brand account. Tracking links are generated
          from a creator/publisher account.
        </p>
      </div>
    );
  }
  ...
  if (generated) {
    return (
      <div className="card border-primary/30 p-5 ring-1 ring-primary/10">
        ...
        {/* The three facts a publisher checks before they post: what they are
            about to share, what it pays, and which campaign it belongs to. */}
        <dl className="mt-4 divide-y divide-border rounded-md border border-border">
          <SummaryRow label="Your link">
            <span className="truncate font-mono text-xs">
              {shortUrl(generated.url)}
            </span>
          </SummaryRow>
          <SummaryRow label="Payout">{payoutDescription}</SummaryRow>
          <SummaryRow label="Campaign">
            {campaignName} <span className="text-fg-subtle">· {brandName}</span>
          </SummaryRow>
        </dl>
        ...
        <p className="mt-3 text-xs text-fg-subtle text-pretty">
          Advertising disclosure rules vary by country and platform. Complying
          with the ones that apply to you is your responsibility — this is not
          legal advice.
        </p>
    ...
    <div className="card p-5">
      ...
      {budgetExhausted ? (
        <Alert tone="warning" className="mt-4" title="Budget exhausted">
          This campaign has spent its funded budget. You can still take a link,
          but traffic will not earn until the brand adds funds.
        </Alert>
      ) : null}
      ...
      <ActionForm
        ...
        refresh={false}
        onSuccess={(data) =>
          setGenerated({ url: data.url, reused: data.reused })
        }
      >
...
function ApplyPanel({
  ...
  if (applicationStatus === "PENDING") {
    return (
      <div className="card p-5">
        ...
        <h2 className="text-md font-semibold text-fg">
          Waiting on {brandName}
        </h2>
        <p className="mt-1.5 text-sm text-fg-muted text-pretty">
          Your application to promote {campaignName} is being reviewed. You will
          be notified when the brand responds, and your link will be available
  ...
  if (applicationStatus === "REJECTED") {
    return (
      <div className="card p-5">
        ...
        <p className="mt-1.5 text-sm text-fg-muted text-pretty">
          {brandName} did not accept your application for this campaign. Plenty
          of open campaigns need no approval at all.
        </p>
        <ButtonLink
  ...
  return (
    <div className="card p-5">
      ...
      <p className="mt-1.5 text-sm text-fg-muted text-pretty">
        {brandName} reviews publishers before issuing links for this campaign.
        Tell them how you plan to promote it.
      </p>

      <ActionForm
        action={applyToCampaign}
        csrfToken={csrfToken}
        className="mt-4"
      >
        <input type="hidden" name="campaignId" value={campaignId} />
...
export function CopyField({
  ...
  return (
    <div className={className}>
      ...
      <p
        role="status"
        className={`mt-2 text-xs text-primary transition-opacity ${copied ? "opacity-100" : "opacity-0"}`}
      >
...
function SummaryRow({
  ...
  return (
    <div className="flex items-baseline gap-3 px-3 py-2">
      <dt className="w-20 shrink-0 text-2xs font-medium uppercase tracking-wide text-fg-subtle">
        {label}
      </dt>
      <dd className="min-w-0 flex-1 truncate text-sm text-fg">{children}</dd>
    </div>
  );
}

/** The link without its scheme — the form a publisher would read aloud. */
