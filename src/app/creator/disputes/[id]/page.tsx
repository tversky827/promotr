...
export default async function CreatorDisputePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { creator, user } = await pageCreator();
  const csrfToken = await currentCsrfToken();

  const loaded = await loadDispute(id, user.id, false);
  // Scope check: a publisher may only view their own disputes.
  if (!loaded || loaded.dispute.creatorId !== creator.id) notFound();
  ...
  return (
    <>
      <PageHeader
        breadcrumb={
          <Breadcrumb
            items={[{ label: 'Disputes', href: '/creator/disputes' }, { label: dispute.reference }]}
          />
        }
          ...
          <Card>
            ...
            <DescriptionList columns={1} className="mt-4">
              ...
              {dispute.targetIds.length > 0 ? (
                <Field label="Records referenced">{dispute.targetIds.length}</Field>
              ) : null}
              {dispute.resolvedAt ? (
                <Field label="Resolved">{formatDateTime(dispute.resolvedAt)}</Field>
              ) : null}
            </DescriptionList>
          </Card>
        </div>
